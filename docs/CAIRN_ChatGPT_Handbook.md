...
- **`failed`-Sessions in `reconcile_training_block`:** Ein fehlgeschlagener Push setzt nie eine `garmin_workout_id` (weder in `training_plan` noch in `garmin_mcp_log`) — es gibt nichts zu löschen, `delete_garmin_workout` ist mangels ID gar nicht aufrufbar. Stattdessen denselben Call mit derselben `external_id`/`session_id` erneut ausführen (`create_garmin_workout` bzw. `push_sessions_to_garmin`) — der alte `garmin_mcp_log`-Eintrag mit `status='failed'` blockiert den Retry nicht (Idempotenz-Check greift nur bei `status='success'`) und wird bei Erfolg überschrieben.
